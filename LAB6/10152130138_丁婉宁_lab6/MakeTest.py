...
def outputFile(tfidfSort, bm25Sort, queryname):
	...
	for key, value in tfidfSort:
		...
		bmS = bmDic[key]
		writeStr = str(queryname) + ' ' + str(keylist) + ' ' + str(value) + ' ' + str(tfidfIndex) + ' ' + str(
				bmS) + ' ' + str(keyIndex) + '\n'
		f.write(writeStr)
	f.close()
...
def loadQuery(foldername):
	testList = []
	f = open(queryDir[1] + '/' + foldername + '.title')
	test = ''
	for words in f:
		test = test + ' ' + words
	while '_' in test or '`' in test or '.' in test or '-' in test or '!' in test or ';' in test or ',' in test or '$' in test or '//' in test or '/' in test or '{' in test or '}' in test or '*' in test or '#' in test or '^' in test or '|' in test or '~' in test or '=' in test or '\'' in test or '+' in test or ':' in test or '?' in test or 'CD' in test:
		test = test.replace('CD', "").replace("_", "").replace('`', "").replace(".", "").replace('-', "").replace("!",
		                                                                                                          "").replace(
...
if __name__ == '__main__':
	f = open('10152130138_丁婉宁_test', 'a')
	f.write('queryname' + ' document' + ' tfidf' + ' tfidfRank' + ' bm25' + ' bm25Rank\n' )
	f.close()
	fileFolder()
	getStopword()

	i = 0
	for foldername in testFolder:
		print(i, len(testFolder), foldername)
		i = i + 1
		query = loadQuery(foldername)
		loadDocument(foldername, docDir[1], query)
