...
dirlist=["./XML_AP","./XML_ZF","./XML_WJS","./XML_DOE","./XML_FR"]
word_dic={}#单词的倒排索引字典
...
if __name__ == '__main__':
	...
	f_BM25 = open('10152130138_丁婉宁_BM25.res', 'w')
	for key,value in score_BM25.items():
		valueSort_BM25=sorted(value.items(),key=lambda d:d[1],reverse=True)#按照score_BM25倒序
		t=0
		for key1, value1 in valueSort_BM25:
			f_BM25.write(key+' '+'0'+' '+key1+' '+str(t)+' '+str(value1)+' '+'10152130138_BM25'+'\n')
			t=t+1
	f_BM25.close()
	for i in range(len(query)):
		getTfIdf(query[i],query_id[i])
	f_tfidf = open('10152130138_丁婉宁_tfidf.res', 'w')
	for key,value in score_tfidf.items():
		valueSort_tfidf=sorted(value.items(),key=lambda d:d[1],reverse=True)#按照score_tfidf倒序
		t=0
		for key1, value1 in valueSort_tfidf:
			f_tfidf.write(key+' '+'0'+' '+key1+' '+str(t)+' '+str(value1)+' '+'10152130138_tfidf'+'\n')
			t=t+1
	f_tfidf.close()
