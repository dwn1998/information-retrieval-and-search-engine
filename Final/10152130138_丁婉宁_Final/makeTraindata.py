...
def outputFile(tfidfSort,bm25Sort,queryname):
	...
	for key,value in tfidfSort:
		...
		if keylist in trainQrels[queryname]:
			...
			writeStr=str(queryname) +' '+str(keylist)+' ' +str(tfidfIndex)+' '+str(keyIndex)+' 1\n'
		else:
			writeStr = str(queryname) + ' ' + str(keylist) + ' ' + str(tfidfIndex) + ' ' + str(keyIndex) + ' 0\n'
		f.write(writeStr)
	f.close()
...
def loadQuery(foldername):
	try:
		...
		print(queryDir[0] + '/' + foldername + '.title')
