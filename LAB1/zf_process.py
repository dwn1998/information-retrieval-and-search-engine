...
import gzip
import tarfile
rootdir1="./disk12/disk1/ZIFF"
rootdir2="./disk12/disk2/ZIFF"
stopwords=[]
docno_zf=[]
docid_zf=[]
text_zf=[]
...
def tokenization(str_test):
    ...
    for word in str_test:
        word_token.append(nltk.word_tokenize(word))
...
def stemmed(str_test):
    ...
    for words in str_test:
        ...
        test.append(s)
    return test
def zf_getdata(str_dir,filename):
    dir_filename=str_dir+filename
    zf_open=open(dir_filename,"r")
    file=open(dir_filename+".xml","w")
    zf=""
    for e in zf_open:
        zf=zf+e.replace("&","").replace("#","").replace("@","")
    file.write("<document>\n"+zf+"</document>")
    file.close()
    zf_open.close()
    ...
    for e in xmldoc.findall('DOC'):
        for s1 in e.findall('DOCNO'):
            docno_zf.append(s1.text.strip())
        for s7 in e.findall('TEXT'):
            text_zf.append(s7.text)   
    zf_processing(str_dir,filename)
    
def write_file_zf(text,str_dir,filename):
    t=0
    dir_filename=str_dir+filename
    xmldoc = ET.parse(dir_filename+".xml")
    for e in xmldoc.findall('DOC'):
    ...
    s=0
    for words in zf:
        query="".join(docno_zf[s])
        file_test=open('./XML_ZF/'+query+".xml",'w')
        word="".join(words)
        file_test.write(word)
        s=s+1
        file_test.close()
    file.close()
def zf_processing(str_dir,filename):
    dir_filename=str_dir+filename
    text_token=tokenization(text_zf)
    text_stopword=stopword(text_token)
    text_lemmatize=lemmatization(text_stopword)
    text=stemmed(text_lemmatize)
    write_file_zf(text,str_dir,filename)
if __name__ == '__main__':
    ...
    for word in filename1:
        f_name=word
        g_file=gzip.GzipFile(rootdir1+'/'+word+".gz")
        open("./disk_ZF/"+f_name,"w+").write(g_file.read().decode())
        g_file.close()
    ...
    for word in filename2:
        f_name=word
        g_file=gzip.GzipFile(rootdir2+'/'+word+".gz")
        open("./disk_ZF/"+f_name,"w+").write(g_file.read().decode())
        g_file.close()
    
    for parent,dirnames,filenames in os.walk("./disk_ZF"):
        i=0
        for e in filenames:
            filename.append(e)
            zf_getdata("./disk_ZF/",e)
            docno_zf=[]
            text_zf=[]
            i=i+1
