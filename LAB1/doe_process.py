...
def stemmed(str_test):
    ...
    for words in str_test:
        ...
        test.append(s)
    return test
def doe_getdata(str_dir,filename):
    dir_filename=str_dir+filename
    doe_open=open(dir_filename,"r")

    file=open(dir_filename+".xml","w")

    doe=""
    for e in doe_open:
        doe=doe+e.replace("&","").replace("#","").replace("@","")
    file.write("<document>\n"+doe+"</document>")
    file.close()
    doe_open.close()
    ...
    for e in xmldoc.findall('DOC'):
        for s1 in e.findall('DOCNO'):
            docno_doe.append(s1.text.strip())
        for s7 in e.findall('TEXT'):
            text_doe.append(s7.text)   
    doe_processing(str_dir,filename)

def write_file_doe(text,str_dir,filename):
    t=0
    dir_filename=str_dir+filename
    xmldoc = ET.parse(dir_filename+".xml")
    for e in xmldoc.findall('DOC'):
    ...
    s=0
    for words in doe:
        query="".join(docno_doe[s])
        file_test=open('./XML_DOE/'+query+".xml",'w')
        word="".join(words)
        file_test.write(word)
        s=s+1
        file_test.close()
    file.close()
def doe_processing(str_dir,filename):
    text_token=tokenization(text_doe)
    text_stopword=stopword(text_token)
    text_lemmatize=lemmatization(text_stopword)
    text=stemmed(text_lemmatize)
    write_file_doe(text,str_dir,filename)

if __name__ == '__main__':
    filename1=[]
    filename=[]
    get_stopword()
    for parent,dirnames,filenames in os.walk(rootdir1_doe):
        for e in filenames:
            filename1.append(e.replace(".gz",""))
    for word in filename1:
        f_name=word
        g_file=gzip.GzipFile(rootdir1_doe+'/'+word+".gz")
        open("./disk_DOE/"+f_name,"w+").write(g_file.read().decode("utf8","ignore"))
        g_file.close()
    for parent,dirnames,filenames in os.walk("./disk_DOE"):
        for e in filenames:
            filename.append(e)
            doe_getdata("./disk_DOE/",e)
            docno_doe=[]
            text_doe=[]
